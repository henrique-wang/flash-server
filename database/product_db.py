...
# Receive Product oldProduct, Product newProduct
# Edit product from products.csv
# Return boolean True, if product was found
def editProduct(oldProduct, newProduct):
    dataBase = pd.read_csv('products.csv')
    for i in range(len(dataBase)):
        currName = dataBase["ProductName"][i]
        if (oldProduct.getName() == currName):
            # Change Product Price
            dataBase["ProductPrice"][i] = newProduct.getPrice()
            dataBase["ProductName"][i] = newProduct.getName()
            # Save new value
            dataBase.to_csv('./products.csv', index = False)
            return True
    return False
