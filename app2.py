...
def database():
    if request.method == 'GET':
        ...
        for product in products:
            list.append({"product": {"name": product.getName(), "price": product.getPrice()}})
...
@app.route('/api/database/edit', methods=['POST'])
def editProduct():
    frame = request.json
    print(frame)
    #frame = json.loads(content)
    reqOldProduct = frame["oldproduct"]
    oldProduct = Product(999, reqOldProduct["name"], float(reqOldProduct["price"]))
    reqNewProduct = frame["newproduct"]
    newProduct = Product(999, reqNewProduct["name"], float(reqNewProduct["price"]))
    editOk = db.editProduct(oldProduct, newProduct)
    if (editOk):
        products = db.getAllProducts()
        list = []
        for product in products:
        ...
        res = {"productList": "Product not found"}
...
def prediction():
    ...
    # Load the network with YOLOv4 weights and config using darknet framework
